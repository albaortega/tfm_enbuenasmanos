...
export class HistorialComponent implements OnInit {
    ...
    constructor( private serviceService: ServiceService,
        ...
        this.listService.filterPredicate = ((data, filter) => {
            ...
            const c = !filter.text || this.userInFilter(data.user,filter.text) || data.client.toLowerCase().includes(filter.text) ||this.workerInFilter(data.worker,filter.text)  || data.address.toLowerCase().includes(filter.text);
    ...
    loadServices(){
        let servicesArray = [];
        this.serviceService.getServices({'email': this.userInfo.email,'filter':'client'}).subscribe(
            (services: Service[])=>{
    ...
    changeService(id: any, status: string) {
        //this.openLoader();
        let params = {'id': id, 'data':{ 'status': status}};
        this.confirmService.confirm({title:'Confirmar',message:'¿Estás seguro que deseas cancelar ese servicio?'}).subscribe(result=>{
            if(result){
                this.serviceService.changeStatusService(params).subscribe(
                    (service: Service)=>{
                        this.openPopUp('Servicio cancelado correctamente','success');
                        this.loadServices();
                    }, error => { console.error(error); }
                );
            }
